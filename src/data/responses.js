/**
 * The task prompt used across both experimental conditions.
 */
...
/**
 * Generates response segments whose text and confidence vary based on the
 * four parameter sliders. Each segment has:
 *  - id, category, label: identification
 *  - text: content (switches based on parameter values)
 *  - confidence: 0–1 score (shifts with parameters)
...
export function generateSegments(params) {
  ...
  const segments = [
    // ── Opening / Approach ──
    {
      ...
      confidence: 0.92 - cre * 0.15 + eff * 0.05,
      text:
        cre > 0.65
    ...
    // ── Partition Handling ──
    {
      ...
      confidence: 0.82 - cre * 0.12 + det * 0.08,
      text:
        det > 0.6
    ...
    // ── Creative Optimization (conditional) ──
    ...(cre > 0.4
      ? [
          {
            id: 's5',
            ...
            confidence: 0.58 + eff * 0.15 - cre * 0.1,
            text:
              cre > 0.7
    ...
    // ── Pseudocode (conditional on detail) ──
    ...(det > 0.35
      ? [
          {
            id: 's6',
  ...
  return segments.map((s) => ({
    ...s,
    confidence: Math.max(0.15, Math.min(0.99, s.confidence)),
  }))
}

/**
 * Pre-canned chat responses for Condition A (control).
 * Simulates a standard AI assistant responding to follow-up prompts.
 */
export const CHAT_RESPONSES = [
  {
    trigger: null, // initial response to the task prompt
